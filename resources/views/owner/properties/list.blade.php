...
<div class="d-flex justify-content-between align-items-center mb-4">
    ...
    <a href="{{ route('owner.properties.create') }}" class="btn" style="background: var(--accent-gold); color: #0F1A3C; font-weight: 600; border-radius: 10px; padding: 10px 20px;">
...
<ul class="nav nav-tabs">
    ...
    <li class="nav-item">
        <a class="nav-link" href="{{ route('owner.earnings') }}">Earnings</a>
    </li>
    <li class="nav-item">
        <a class="nav-link" href="{{ route('owner.documents') }}">Documents</a>
    </li>
    <li class="nav-item">
        <a class="nav-link" href="{{ route('owner.analytics') }}">Analytics</a>
    </li>
</ul>
                ...
                <tr>
                    <td class="ps-3">
                        <div class="d-flex align-items-center gap-3">
                            ...
                            <div>
                                <h6 class="text-white mb-0 fw-bold">{{ Str::limit($property->title, 25) }}</h6>
                                <small class="text-muted" style="font-size: 11px;">ID: #{{ $property->id }}</small>
                            </div>
                        </div>
                    </td>
                    <td>
                        <span class="text-white">{{ Str::limit($property->location, 20) }}</span>
                    </td>
                    <td>
                    ...
                    <td class="fw-bold text-white">₦{{ number_format($property->price) }}</td>
                    <td class="text-muted">{{ $property->created_at->format('M d, Y') }}</td>
                    <td class="text-end pe-3">
                        <a href="{{ route('properties.show', $property->id) }}" class="action-btn me-1" title="View Public Page"><i class="bi bi-eye"></i></a>
                        
                        @if($property->canBeEditedBy(auth()->id()))
                            <a href="{{ route('owner.properties.edit', $property->id) }}" class="action-btn me-1" title="Edit"><i class="bi bi-pencil"></i></a>
                        @endif
                        
                        @if($property->canBeDeletedBy(auth()->id()))
                            <form action="{{ route('owner.properties.delete', $property->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Delete this property permanently?');">
                                @csrf
                ...
                <tr>
                    <td colspan="6" class="text-center py-5">
                        <div class="d-flex flex-column align-items-center">
                            ...
                            <a href="{{ route('owner.properties.create') }}" class="btn btn-sm mt-2" style="background: var(--accent-gold); color: #0F1A3C; font-weight: 600;">create One Now</a>
                        </div>
                    </td>
                </tr>
                @endforelse
