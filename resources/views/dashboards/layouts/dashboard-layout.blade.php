...
<head>
    ...
    <title>@yield('title', 'Dashboard') - 360WinEstate</title>
...
<body>
    ...
    <div class="sidebar" id="sidebar">
        ...
        <ul class="sidebar-menu">
            <li><a href="{{ route('dashboard') }}" class="{{ request()->routeIs('dashboard*') || request()->routeIs('*.dashboard') ? 'active' : '' }}">
                <i class="bi bi-grid-fill"></i> Dashboard
            </a></li>

            @if(auth()->user()->isOwner())
                <li><a href="{{ route('owner.properties') }}" class="{{ request()->routeIs('owner.properties*') ? 'active' : '' }}">
                    <i class="bi bi-houses-fill"></i> My Properties
                </a></li>
                <li><a href="{{ route('owner.earnings') }}" class="{{ request()->routeIs('owner.earnings*') ? 'active' : '' }}">
                    <i class="bi bi-cash-stack"></i> Earnings
                </a></li>
                <li><a href="{{ route('owner.documents') }}" class="{{ request()->routeIs('owner.documents*') ? 'active' : '' }}">
                    <i class="bi bi-file-earmark-text"></i> Documents
                </a></li>
                <li><a href="{{ route('owner.analytics') }}" class="{{ request()->routeIs('owner.analytics*') ? 'active' : '' }}">
                    <i class="bi bi-graph-up"></i> Analytics
                </a></li>
            @endif

            @if(auth()->user()->isMarketer())
                <li><a href="{{ route('marketer.team') }}" class="{{ request()->routeIs('marketer.team*') ? 'active' : '' }}">
                    <i class="bi bi-people-fill"></i> My Team
                </a></li>
                <li><a href="{{ route('marketer.commissions') }}" class="{{ request()->routeIs('marketer.commissions*') ? 'active' : '' }}">
                    <i class="bi bi-wallet-fill"></i> Commissions
                </a></li>
                <li><a href="{{ route('marketer.leaderboard') }}" class="{{ request()->routeIs('marketer.leaderboard*') ? 'active' : '' }}">
                    <i class="bi bi-trophy-fill"></i> Leaderboard
                </a></li>
                <li><a href="{{ route('marketer.targets') }}" class="{{ request()->routeIs('marketer.targets*') ? 'active' : '' }}">
                    <i class="bi bi-bullseye"></i> Targets
                </a></li>
            @endif

            @if(auth()->user()->isInvestor())
                <li><a href="{{ route('investor.portfolio') }}" class="{{ request()->routeIs('investor.portfolio*') ? 'active' : '' }}">
                    <i class="bi bi-briefcase-fill"></i> Portfolio
                </a></li>
                <li><a href="{{ route('investor.investments') }}" class="{{ request()->routeIs('investor.investments*') ? 'active' : '' }}">
                    <i class="bi bi-graph-up-arrow"></i> Investments
                </a></li>
                <li><a href="{{ route('investor.dividends') }}" class="{{ request()->routeIs('investor.dividends*') ? 'active' : '' }}">
        ...
        <div class="dashboard-header">
            <div class="d-flex align-items-center gap-3">
                ...
                <div class="welcome-text">
                    <h2>Welcome, {{ explode(' ', auth()->user()->name)[0] }} <span>👋</span></h2>
                </div>
            </div>
